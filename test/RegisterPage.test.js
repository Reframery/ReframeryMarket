...
describe("RegisterPage UI and functions testing", () => {
  describe("field tests in register page", () => {
    ...
    it("should have a Welcome Component", () => {
      ...
      const welcomeComponent = registerPageWrapper.find(WelcomeComponent)
    ...
    it("should have a checkbox to chooose a community in the range of Canada, USA, Brazil, mexico", () => {
      ...
      expect(registerPageWrapper.find("select").text().includes("Canada")).toBe(
        true
      )
      expect(registerPageWrapper.find("select").text().includes("USA")).toBe(
        true
      )
      expect(registerPageWrapper.find("select").text().includes("Brazil")).toBe(
        true
      )
      expect(registerPageWrapper.find("select").text().includes("Mexico")).toBe(
        true
      )
    })
  ...
  describe("RegisterPage function testing", () => {
    ...
    it("shuold set all values on changes", () => {
      ...
      const confirmPasswordInput = simulateChangeOnInput(
        registerPageWrapper,
        "#confirmPassword",
        "1234"
      )
      const communityInput = simulateChangeOnInput(
        registerPageWrapper,
        "select",
        "Canada"
      )
