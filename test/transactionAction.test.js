...
} from "redux/constants/transactionConstants"
import {
  getTransaction,
...
describe("Transaction actions testing", () => {
  ...
  describe("getTransaction action", () => {
    it("should get the transaction record sucessfully if the transaction exists", async () => {
      mock
        .onGet("/transactions/transaction/user02@gmail.com")
  ...
  describe("getTransactionsOfUser action", () => {
    it("should get the transaction list sucessfully if the user exists", async () => {
      mock
        .onGet(
          "/transactions/getTransactionsOfUser/user02@gmail.com-10-1-reversed"
        )
        .reply(200, data.transactions)
      await store.dispatch(
        getTransactionsOfUser("user02@gmail.com", 10, 1, "reversed")
      )
      const actions = store.getActions()
      const expectedPayload = {
        payload: data.transactions,
        type: TRANSACTION_OF_USER_GET_SUCCESS,
      }
      expect(actions[1]).toEqual(expectedPayload)
    })
    ...
    it("should fail to get the transaction list if the user does not exit", async () => {
      mock
        .onGet(
          "transactions/getTransactionsOfUser/user04@gmail.com-10-1-reversed"
        )
        .reply(404, { message: "" })
      await store.dispatch(
        getTransactionsOfUser("user04@gmail.com", 10, 1, "reversed")
      )
      const actions = store.getActions()
      const expectedPayload = {
        payload: "Request failed with status code 404",
  ...
  describe("getBalance action", () => {
    ...
    it("should fail to get the transaction record if the user does not exit", async () => {
      mock
        .onGet("transactions/balance/user04@gmail.com")
        .reply(404, { message: "" })
  ...
  describe("addBalanceToUser action", () => {
    it("should add balance to user sucessfully if the user exists", async () => {
      mock
        .onPost("/transactions/addBalanceToUser")
        .replyOnce(200, data.transactions[1])
      await store.dispatch(
        addBalanceToUser("manager01@gmail.com", "user02@gmail.com", 25)
      )
      const actions = store.getActions()
      const expectedPayload = {
        payload: data.transactions[1],
        type: BALANCE_ADD_SUCCESS,
      }
      expect(actions[1]).toEqual(expectedPayload)
    })
    ...
    it("should fail to add balance to user if the user does not exit and return an error message", async () => {
      mock
        .onPost("transactions/addBalanceToUser")
        .replyOnce(404, { message: "This user does not exists" })
      await store.dispatch(
        addBalanceToUser("manager01@gmail.com", "user04@gmail.com", 25)
      )
      const actions = store.getActions()
      const expectedPayload = {
        payload: "This user does not exists",
  ...
  describe("deductBalanceToUser action", () => {
    it("should deduct a balance from user sucessfully if the user has engough credits", async () => {
      mock
        .onPost("/transactions/deductBalanceFromUser")
        .replyOnce(200, data.transactions[2])
      await store.dispatch(
        deductBalanceFromUser("manager01@gmail.com", "user02@gmail.com", 5)
      )
      const actions = store.getActions()
      const expectedPayload = {
        payload: data.transactions[2],
        type: BALANCE_DEDUCT_SUCCESS,
      }
      expect(actions[1]).toEqual(expectedPayload)
    })
    ...
    it("should fail to deduct balance from a  user if the user does not have enough credits", async () => {
      mock
        .onPost("transactions/deductBalanceFromUser")
        .replyOnce(406, { message: "Required Attribute Not Satisfied" })
      await store.dispatch(
        deductBalanceFromUser("manager01@gmail.com", "user04@gmail.com", 30)
      )
      const actions = store.getActions()
      const expectedPayload = {
        payload: "Required Attribute Not Satisfied",
